...
const Income = () => {
  ...
  const { totalIncome, incomesArray } = useSelector((s) => s.accountData)

  const [open, setOpen] = React.useState(false);
  const handleOpen = () => setOpen(true);

  const calculatePercentage = () => {
    const latestTransaction = incomesArray[incomesArray.length - 1]

    if (latestTransaction) {
      const percentage = (latestTransaction / totalIncome) * 100;
